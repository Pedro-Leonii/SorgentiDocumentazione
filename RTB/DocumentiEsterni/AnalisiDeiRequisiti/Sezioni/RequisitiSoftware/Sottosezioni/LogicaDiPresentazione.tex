...
\begin{swreq}
    ...
    \subreq{RFO-1.06}{Se il dataset caricato non è vuoto il sistema deve visualizzare una barra di ricerca che specifichi all'utente le informazioni necessarie per l'esecuzione della stessa}
    ...
    \subreq{RFO-1.11}{Se il dataset caricato contiene delle modifiche non ancora salvate il sistema deve visualizzare un pulsante che permette il salvataggio dello stesso}
...
\begin{swreq}
    ...
    \subreq{RFO-2.02}{Se il sistema verifica che è stata richiesta una pagina invalida visualizza un messaggio di errore}
        
    \subreq{RFO-2.03}{Se il sistema riscontra un errore durante l'ottenimento degli elementi appartenenti alla pagina deve visualizzare un messaggio di errore}
...
\begin{swreq}
    ...
    \subreq{RFO-3.06}{Se l'utente richiede l'eliminazione di un elemento il sistema richiedere la conferma}

\end{swreq}

\begin{swreq}
    {RFO-4}
    {Il sistema deve poter visualizzare gli elementi per l'inserimento/modifica di un elemento nel dataset caricato}
    \label{rf:RFO-4}%
    ...
    \subreq{RFO-4.05}{Se si cerca di confermare un elemento invalido il sistema deve visualizzare un messaggio di errore in cui viene indicato come compilare correttamente l'elemento}
...
\begin{swreq}
    {RFO-6}
    {Il sistema deve poter visualizzare gli elementi necessari per nominare/rinominare un dataset}
    \label{rf:RFO-6}%
    ...
    \subreq{RFO-6.04}{Se si cerca di confermare un elemento invalido il sistema deve visualizzare un messaggio di errore}
...
\begin{swreq}
    ...
    \subreq{RFO-8.09}{Se il sistema riscontra un errore interno durante l'ottenimento delle informazioni del dataset salvato deve visualizzare un messaggio di errore}

 \end{swreq}

 \begin{swreq}
    [
        \dependency{Se il sistema può gestire un insieme di test salvati deve indicare il nome del test caricato se non è temporaneo altrimenti indicare il fatto che è temporaneo}
 ...
 \begin{swreq}
    ...
    \subreq{RFO-12.03}{Se il sistema verifica che è stata richiesta una pagina invalida visualizza un messaggio di errore}
 ...
 \begin{swreq}
    ...
    \subreq{RFO-13.03}{Il sistema deve visualizzate la risposta ottenuta}
...
\begin{swreq}
    ...
    \subreq{RFF-2.04}{Se l'utente richiede l'eliminazione di un LLM associato a uno o più test salvati il sistema deve notificare che i test verranno eliminati}
...
\begin{swreq}
    ...
    \subreq{RFF-3.07}{Il sistema deve visualizzare la chiave da utilizzare per estrarre la risposta data dall'LLM contenuta risposte HTTP}
...
\begin{swreq}
    ...
    \subreq{RFF-4.11}{Se si cerca di salvare un URL con formato invalido il sistema deve mostrare un messaggio di errore}

    \subreq{RFF-4.12}{Se si cerca di salvare una coppia chiave valore invalida il sistema deve mostrare un messaggio di errore}
...
\begin{swreq}
    {RFF-5}
    {Il sistema dovrebbe poter visualizzare gli elementi necessari alla creazione di un nuovo LLM}
    \label{rf:RFF-5}%
    ...
    \subreq{RFF-5.09}{Se si cerca di salvare un URL con formato invalido il sistema deve mostrare un messaggio di errore}

    \subreq{RFF-5.10}{Se si cerca di salvare una coppia chiave valore invalida il sistema deve mostrare un messaggio di errore}
...
\begin{swreq}
    ...
    \subreq{RFF-8.04}{Se si cerca di utilizzare un nome invalido il sistema deve visualizzare un messaggio di errore}
...
\begin{swreq}
    ...
    \subreq{RFF-9.01}{Il sistema deve visualizzare sotto forma di lista i test salvati che possono essere confrontati con il test caricato indicando nome del dataset di test, data di esecuzione e nome dell'LLM utilizzato}
...
\begin{swreq}
    ...
    \subreq{RFO-11.02}{Il sistema deve visualizzare un diagramma di dispersione che rappresenta i singoli risultati confrontati nella pagina \hyperref[rf:RFO-14]{RFO-14}}

    \subreq{RFO-11.03}{Se il sistema verifica che è stata richiesta una pagina invalida visualizza un messaggio di errore}
...
\begin{swreq}
    ...
    \subreq{RFO-12.01}{Il sistema deve visualizzare domanda, risposta ottenuta e risposta attesa del primo risultato}

    \subreq{RFO-12.02}{Il sistema deve visualizzare domanda, risposta ottenuta e risposta attesa del secondo risultato}
...
\begin{swreq}
    ...
    \subreq{RFO-13.03}{Se il file JSON non rispecchia il formato supportato il sistema deve generare un messaggio di errore}
