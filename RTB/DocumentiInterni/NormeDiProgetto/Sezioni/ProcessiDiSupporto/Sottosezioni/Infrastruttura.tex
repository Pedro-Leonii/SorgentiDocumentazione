...
Un ramo è un etichetta mobile nel senso che Git la sposta in automatico quando viene gerato un commit figlio.
...
\begin{enumerate}
    ...
    \item \textbf{Albero di lavoro}: albero di cartelle e file che Git "conosce" e quindi ritiene come facenti parti del repository.
...
\subparagraph{pull}
\label{subpar:pull}
L'operazione di push permette aggiornare un repository locale con le modifiche registrate al repository remoto a cui è associato.
...
Git indica questa situazione nella console in cui appare la keywork \texttt{MERGING} che indica il fatto che siamo nel mezzo di un operazione di fusione.
...
\begin{itemize}
    \item \textbf{action}: una GitHub action pubblicata su GitHub action marketplace che lo step chiama.
    Il markeplace delle GitHub action si trova al link \href{https://github.com/marketplace?type=actions}{actions}.
